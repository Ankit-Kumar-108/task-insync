...
export default function ClockCalendar() {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-sm mx-auto">
      ...
      {/* 📅 CALENDAR CARD */}
      <div className="bg-white/80 backdrop-blur-xl border border-white/50 p-6 rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-lg font-bold text-gray-800">
            {date.toLocaleDateString("en-US", { month: "long" })} <span className="text-indigo-600">{date.getFullYear()}</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-7 text-center gap-2">
          {/* Weekday Headers */}
          {weekDays.map((day) => (
            <div key={day} className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">
              {day}
            </div>
          ))}
