...
// Define Task interface locally to avoid circular deps (should match TasksContainer)
interface Task {
  id: string;
  title: string;
...
export default function AddTaskInput({ title, onOptimisticAdd }: AddTaskInputProps) {
  ...
  return (
    <div className="mb-8 group focus-within:ring-2 ring-[#2b8cee] ring-offset-2 ring-offset-[#f6f7f8] dark:ring-offset-[#101922] rounded-xl">
      ...
      <form
        action={async (formData) => {
          ...
          if (onOptimisticAdd) {
            ...
            onOptimisticAdd(newTask);
          }

          setIsLoading(true)
          await CreateTask(formData);
          setTaskName(""); // Clear text
          resetDate(); // Reset date to default
          setIsLoading(false)
      ...
      >
        ...
        <button type="submit" disabled={isLoading} className="flex h-full w-14 bg-blue-500/20 shrink-0 items-center justify-center text-[#2b8cee] hover:bg-blue-500/50 dark:hover:bg-blue-500/50 transition">
          {isLoading ? (
            <Loader />
          ) : (
